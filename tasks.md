...
- [~] 데이터 수집 상태 점검
  - [~] 채용공고 데이터 5,000건 이상 확보 (진행 중: 1,986건/5,000건, 39.7%)
    - SENIOR_KEYWORDS 필터 제거 완료
    - 크롤링 범위 확대: 9개 카테고리 × 20페이지 = 500+ URL/배치
    - 배치 크기: 2,000건으로 증가
    - 백그라운드 실행 중 (PID: 26372, 로그: /tmp/saramin_crawler_v2.log)
    - 현재: 1,986건 (474개 회사) - 1분당 약 268건 수집 중
  - [~] 자기소개서 데이터 1,000건 이상 확보 (진행 중: 312건/1,000건, 31.2%)
    - Linkareer 크롤러 실행 중 (PID: 61096)
    - Playwright 기반 비동기 크롤링
    - 백그라운드 실행 중 (로그: /tmp/linkareer_crawler.log)
    - 현재: 312건 (167개 회사) - 1분당 약 160건 수집 중
  - [ ] 데이터 품질 점검 및 정제
